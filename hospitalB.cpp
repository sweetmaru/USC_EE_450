...
int main(int argc, const char * argv[]) {
    ...
    ListNodeDB* p3 = head3;
//    notification of entering three arguments of hospital B
    if(argc!=4){
    ...
    while (!infile.eof())
    {
        ...
        ListNode* q2 = new ListNode(vex2);  //vertixs of second column
        p2->next = q2;
        p2 = p2->next;
    ...
    int locHos = atoi(argv[1]);// location of hospital B
    int reDestination = 0;   //reindex of client location
    int reLocHos = 0; //location of hospitalA（reindexed)
    double distance = 0; //the distance between hospital B and client
    double score=0;
    ...
    while (1) {
        //calculate availability of hospital B
...
//  calculate shortest distance between hospital and client if they are not at the same location
        if(clientLocation!=locHos){
            ...
            distance = Dijkstra1(G,locHos,clientLocation,reLocHos , reDestination);  //the shortest distance between hospital B and client
            if(distance!=0){
    ...
    //    calculate final score of hospital B
        ...
        string scrDis_str=to_string(score)+" "+to_string(distance)+" "+"31403";
        ...
        if(score!=0 && distance!=0){
            ...
            printf("Hospital B has sent score = None and distance = %f to the Scheduler.\n",distance);
        }
        else if (score!=0 && distance==0){
            printf("Hospital B has sent score = %f and distance = None to the Scheduler.\n",score);
        }
        else printf("Hospital B has sent score = None and distance = None to the Scheduler.\n");
