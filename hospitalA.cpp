...
void symmetry(AMGraph &G, int *vertixNo, int &locHos,int &reLocHos,int &destination,int &reDestination){  //symmetry the original map adjacent matrix
    int *index3;
    int *index4;
...
double Dijkstra1(AMGraph &G, int posHos,int des, int rePosHos, int reDes)                            //Dijstra algorithm for searching the shortest path, source code: https://blog.csdn.net/Carizy/article/details/107280388
{
    ...
    for (int i = 1; i <= G.vex; i++)
    {
        dis[i] =double (G.arcs[rePosHos][i]);
...
int main(int argc, const char * argv[]) {
    ...
    ListNodeDB* p3 = head3;
//    notification of entering three arguments of hospital A
    if(argc!=4){
    ...
    reindex(p1,p2,lineno, G.vex, vertixNo,indexVex1,indexVex2); //reindex the vertixs of map.txt
    ...
    int initialOccupancy =atoi(argv[3]);  //acqiure initial occupancy from terminal input
    double availability = 0;

    int locHos = atoi(argv[1]);  // location of hospital A
    int reDestination = 0;   //reindex of client location
    int reLocHos = 0; //location of hospitalA（reindexed)
    double distance = 0; //the distance between hospital A and client
    double score=0;
   ...
   //bind socket
    ...
    printf("Hospital A is up and running using UDP on port %s.\n",MYPORT);
    printf("Hospital A has total capacity ​%d and initial occupancy ​%d.\n",totalCapacity,initialOccupancy);
    //finish initialization of hospital A
   ...
   //send total capacity and intial occupancy to scheduler
    ...
    while (1) {
        //    calculate availability of hospital A
        ...
        if(availability>0){
            ...
            printf("Hospital A has capacity = ​%d, occupation= ​%d​, availability = None\n",totalCapacity,initialOccupancy);
        }
//        calculate shortest distance between hospital and client if they are not at the same location
        if(clientLocation!=locHos){
            ...
            distance = Dijkstra1(G,locHos,clientLocation,reLocHos , reDestination);  //the shortest distance between hospital A and client
            if(distance!=0){
                printf("Hospital A has found the shortest path to client, distance = ​%f\n",distance);

            }
            else
                printf("Hospital A has found the shortest path to client, distance = ​None\n");
        }
        else{
            printf("Hospital A has found the shortest path to client, distance = None.\n");   //if client at same location as hospital,distance=none
        }
        
//        calculate final score of hospital A
        
        if(availability==0 || distance == 0){
            score = 0;
            printf("Hospital A has the score = ​None\n");
        }
        else{
            score = double (1/(distance*(1.1-availability)));
            printf("Hospital A has the score = ​%f\n",score);
        }
        //send score and distance
        string scrDis_str=to_string(score)+" "+to_string(distance)+" "+"30403";
        ...
        if(score!=0 && distance!=0){
            ...
            printf("Hospital A has sent score = None and distance = %f to the Scheduler.\n",distance);
        }
        else if (score!=0 && distance==0){
            printf("Hospital A has sent score = %f and distance = None to the Scheduler.\n",score);
        }
        else printf("Hospital A has sent score = None and distance = None to the Scheduler.\n");
